	...
	public void run() {
		...
		for (String templateName : Arrays.asList("Архитектор", "Боксёр", "Киберспортсмен", "Кинематографист",
			...
			worker.process(
					...
					titleResolver,
					templateName,
					new ReconsiliationColumn(Arrays.asList("Дата рождения", "Дата_рождения"), DataType.TIME,
							Properties.DATE_OF_BIRTH, x -> timeHelper.parse(Properties.DATE_OF_BIRTH, x)) {
						@Override
					...
					new ReconsiliationColumn(Arrays.asList("Место рождения", "Место_рождения"), DataType.WIKIBASE_ITEM,
							Properties.PLACE_OF_BIRTH, x -> placesHelper.parse(entityByLinkResolver,
									Properties.PLACE_OF_BIRTH, x)) {
						@Override
						public ReconsiliationAction getAction(Collection<ValueWithQualifiers> wikipedia,
								Collection<ValueWithQualifiers> wikidata) {
					...
					new ReconsiliationColumn(Arrays.asList("Место смерти", "Место_смерти"), DataType.WIKIBASE_ITEM,
							Properties.PLACE_OF_DEATH, x -> placesHelper.parse(entityByLinkResolver,
									Properties.PLACE_OF_DEATH, x)) {
						@Override
						public ReconsiliationAction getAction(Collection<ValueWithQualifiers> wikipedia,
								Collection<ValueWithQualifiers> wikidata) {
							return placesHelper.getAction(wikipedia, wikidata);
					...
					new ReconsiliationColumn(Arrays.asList("Подданство", "Гражданство"), DataType.WIKIBASE_ITEM,
							Properties.NATIONALITY, x -> countriesHelper.parse(entityByLinkResolver,
									Properties.NATIONALITY, x)) {
						@Override
						public ReconsiliationAction getAction(Collection<ValueWithQualifiers> wikipedia,
								Collection<ValueWithQualifiers> wikidata) {
